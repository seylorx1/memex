function Wrap() {
  this.start = function (data) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      entry.MARK = this.objectSplit(entry.MARK);
  ...
  this.filter = function (target, db) {
    ...
    if (target == '') {
      ...
      if (target == 'term') {
        ...
        else if (splitTarget[0] == 'mark') {
          // MARKER
          let markRequest = decodeURI(splitTarget[1]);
          for (let i = 0; i < keys.length; i++) {
            let value = db[keys[i]];
            if (typeof value.MARK !== 'undefined') {
              console.log(value.MARK + "/" + markRequest);
              if(value.MARK == markRequest) {
                //Found found the post with the custom ID!
                tempDatabase[keys[i]] = db[keys[i]];
              }
            }
          }
